...
const char *_ecv_array _ecv_null  TmcDriverState::CheckStallDetectionEnabled(float speed) const noexcept
{
	...
	{
		return "driver %u.%u is not in spreadCycle mode";
	}
...
#if SUPPORT_CLOSED_LOOP
# if SINGLE_DRIVER
					(GetMoveInstance().IsClosedLoopEnabled(driverNumber)) ? 256 : standstillCurrentFraction;
# else
...
void TmcDriverState::AppendStallConfig(const StringRef& reply) const noexcept
{
	...
	const float stepsPerMm = GetMoveInstance().DriveStepsPerMm(driverNumber);
...
void TmcDriverState::TransferSucceeded(const uint8_t *rcvDataBlock) noexcept
{
	...
	const uint32_t interval = GetMoveInstance().GetStepInterval(driverNumber, microstepShiftFactor);		// get the full step interval
...
static volatile uint8_t phaseSendData[5 * MaxSmartDrivers];
...
void RxDmaCompleteCallback(CallbackParameter param, DmaCallbackReason reason) noexcept
{
	...
	else
	{
		...
		lastWakeupTime += (StepTimer::StepClockRate * DriversDirectSleepMicroseconds)/1000000;
...
extern "C" [[noreturn]] void TmcLoop(void *) noexcept
{
	...
	for (;;)
	{
		...
		if (needToSetCoilCurrents)
		{
			...
			phaseSendData[0] = REGNUM_5160_X_DIRECT | 0x80;
			StoreBEU32(const_cast<uint8_t*>(phaseSendData + 1), driverStates[0].GetPhaseToSet());
